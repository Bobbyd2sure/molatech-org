...
import { Camera, Play, Users, Calendar, ArrowDown } from "lucide-react"
...
export function GalleryHero() {
  ...
  const heroImages = [
    {
      src: "/placeholder.svg?height=600&width=800&text=JAMB+Distribution+Event",
      title: "JAMB Distribution Event",
      description: "Empowering students with free JAMB forms",
      location: "Lagos, Nigeria",
      date: "March 2024",
    },
    {
      src: "/placeholder.svg?height=600&width=800&text=Mentorship+Session",
    ...
    {
      src: "/placeholder.svg?height=600&width=800&text=Graduation+Ceremony",
            ...
            <div className="space-y-6">
              ...
              {isLoading ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-2xl md:text-3xl text-emerald-200 font-semibold animate-fadeInUp">
                    Capturing Moments That Transform Lives
                  </div>
                </div>
              )}
              ...
              {isLoading ? (
                ...
                <p className="text-xl text-slate-300 leading-relaxed max-w-2xl animate-fadeInUp delay-300">
                  Every photograph tells a story of transformation, hope, and achievement. Explore the visual narrative
                  of our educational journey and the lives we've touched together.
                </p>
              )}
            </div>
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col sm:flex-row gap-4 animate-fadeInUp delay-500">
                <Button
                  ...
                  onClick={() => document.getElementById("featured-gallery")?.scrollIntoView({ behavior: "smooth" })}
                >
                ...
                <Button
                  ...
                  onClick={() => document.getElementById("photo-timeline")?.scrollIntoView({ behavior: "smooth" })}
                >
                ...
                <div className="relative bg-white/10 backdrop-blur-md p-6 rounded-3xl shadow-2xl border border-white/20 transform hover:scale-105 transition-all duration-500">
                  <div className="relative overflow-hidden rounded-2xl">
                    ...
                    <div className="absolute bottom-6 left-6 right-6 text-white">
                      <div className="space-y-2">
                        <h3 className="text-2xl font-bold">{heroImages[currentImage].title}</h3>
                        <p className="text-emerald-200">{heroImages[currentImage].description}</p>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-4 text-sm">
                            <div className="flex items-center space-x-1">
                              <Calendar className="h-4 w-4" />
                              <span>{heroImages[currentImage].date}</span>
                            </div>
                            <div className="flex items-center space-x-1">
                              <Users className="h-4 w-4" />
                              <span>{heroImages[currentImage].location}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    {heroImages.map((_, index) => (
                      <button
                        ...
                        className={`w-3 h-3 rounded-full transition-all duration-300 ${
                          index === currentImage ? "bg-emerald-400 w-8" : "bg-white/30 hover:bg-white/50"
                        }`}
                      />
                    ))}
                  </div>
